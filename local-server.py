...
def get_play_info(page_url):
    ...
    # Extract hidden inputs using regex (simple extraction)
    current_id = ''
    mtype = ''
    e_token = ''
    ...
    token = compute_token(current_id, e_token)
    api_url = f'{BASE_URL}/api/getResN?videoId={current_id}&mtype={mtype or "1"}&token={token}'

    api_req = urllib.request.Request(api_url, headers={'User-Agent': UA})
    with urllib.request.urlopen(api_req, timeout=15) as api_resp:
        api_text = api_resp.read().decode('utf-8', errors='replace')
